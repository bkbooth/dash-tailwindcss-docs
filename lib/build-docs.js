...
function transformDocsHtml(config) {
  ...
  docsPages.forEach(docsPage => {
    ...
    $('head').html(`
      ...
      <link rel="stylesheet" href="/assets/${config.tailwindCssFilename}">
...
function copyDocsCss(config) {
  ...
  const docsAssetsDir = path.join(config.scrapeDir, 'tailwindcss.com/css');
